  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center px-4 py-24 overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl">
        <p className="font-hand text-3xl md:text-4xl text-[color:var(--sage)] mb-4">to my favourite person —</p>
        <h2 className="font-display text-5xl md:text-8xl leading-[1.05] text-shadow-soft">
          Happy Birthday,
          <br />
          <span className="italic text-[color:var(--lily-pink)]">Simi</span> 🎂
        </h2>
        <p className="font-fredoka text-xl md:text-2xl mt-6 text-[color:var(--deep-text)]/80">
          ...
          {arrived ? (
            ...
            <div className="grid grid-cols-4 gap-3 md:gap-6 max-w-2xl mx-auto">
              {[
                ...
                <div
                  key={u.l}
                  className={`paint-card p-4 md:p-6 ${
                    [
                      "",
                      "paint-card-rose",
                      "paint-card-sage",
                ...
                >
                  <div className="font-display text-3xl md:text-5xl text-[color:var(--deep-text)] tabular-nums">
                    {String(u.v).padStart(2, "0")}
                  </div>
                  <div className="font-fredoka text-xs md:text-sm uppercase tracking-widest text-[color:var(--deep-text)]/60 mt-1">
                    {u.l}
                  </div>
                </div>
              ))}
