    ...
    <section ref={ref} className="relative py-24 px-4">
      ...
      <div className="relative max-w-3xl mx-auto pl-6 md:pl-12">
        ...
        <ul className="space-y-12">
          {MILESTONES.map((m, i) => (
            <li key={i} className="reveal-on-scroll relative flex gap-5 md:gap-8 items-start">
              <div
                className="relative shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center -ml-6 md:-ml-12"
                style={{
                  background:
              ...
              >
                <LilySprig size={i === MILESTONES.length - 1 ? 50 : 38} />
              </div>

              <div className="paint-card p-5 md:p-6 flex-1">
                <p className="font-hand text-xl text-[color:var(--sage)]">{m.date}</p>
                <h3 className="font-display text-2xl md:text-3xl mt-1">{m.title}</h3>
                <p className="font-body text-base md:text-lg text-[color:var(--deep-text)]/75 mt-2">
                  {m.note}
                </p>
              </div>
            </li>
